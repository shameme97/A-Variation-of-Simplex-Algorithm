...
class My_Simplex:
    ...
    def ensure_feasibility(self, table):
        while any(table[:-1, -1] < 0):   # checking if solution column has negative value
            minimum_ratio = math.inf
            for row in range(len(table[:-1, 0])):
                b = min(table[row, 1:-1])
                Cb = table[row, -1]
                if (Cb < 0 and b < 0) or (Cb >= 0 and b > 0):   # taking only +ve +ve or -ve -ve to produce +ve ratio
                    ratio = Cb / b
    ...
    def check_infeasibility(self, table):
        ...
        if infeasible:
            ...
            table = self.ensure_feasibility(table)
        return table
...
while test_no <= 100:
    ...
    # enter preferred number of variables and constraints
    number_of_variables = 70
    number_of_constraints = 90
    ...
    for method in range(1, 3):
        ...
        if method == 1:   # The general simplex method
            print("\nGeneral Simplex:")
    ...
    if not create_file:
        ...
        # output_file.writelines("Test|General|Modified|  Both soln equal?|  Better|        Feasible?|\n")
        create_file = True
    output_file = open(s, 'a+')
...
# uncomment to take Input from file
